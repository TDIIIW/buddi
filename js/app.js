...
function showScreen(id, slide = false) {
    ...
    if (currentScreen && currentScreen !== next) {
        ...
        setTimeout(() => {
            if (currentScreen) currentScreen.classList.remove('exit');
        }, 300);
    }
    currentScreen = next;
...
document.getElementById('btn-start').addEventListener('click', () => {
    ...
    if (!income || income < 1) {
        shakeEl(document.getElementById('income-input'));
        return;
    }
...
document.getElementById('btn-save-expense').addEventListener('click', () => {
    ...
    if (!amount || amount <= 0) {
        shakeEl(document.getElementById('amount-value'));
        return;
    }
    if (!desc) {
        shakeEl(document.getElementById('exp-desc'));
        return;
    }
